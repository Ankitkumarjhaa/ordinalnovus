...
async function getUnsignedListingPsbt(
  data: CreateListingPsbtData
): Promise<{ ok: boolean; message: string; unsigned_psbt_base64?: string }> {
  try {
    const response = await axios.post(
      ...
      {
        headers: {
          ApiKey: process.env.API_KEY, // Add your API key here in the headers
